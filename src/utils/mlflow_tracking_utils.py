...
from src.constants import EVAL_FIG_DIR, REPORTS_DIR
...
def setup_experiment(experiment_name: str, mlflow_uri: str):
    ...
    try:
        ...
        if "deleted" in str(e).lower() or "not found" in str(e).lower():
            logger.info(f"Experiment '{full_name}' not found/deleted. Creating a new one.")
            mlflow.set_experiment(full_name)

    logger.info(f"MLflow experiment initialized → {full_name} | URI: {mlflow.get_tracking_uri()}")
