...
from src.constants import ADVANCED_DIR, BASELINE_MODEL_DIR, EVAL_DIR, PROJECT_ROOT
...
def save_hyperparams_bundle(model_name: str, params: dict, score: float):
    ...
    logger.info(f"Best parameters saved for {model_name} → {filepath.relative_to(PROJECT_ROOT)}")
...
def save_model_object(model, model_name: str):
    ...
    with open(filepath, "wb") as f:
        pickle.dump(model, f)
    logger.info(f"Final model object saved for {model_name} → {filepath.relative_to(PROJECT_ROOT)}")
...
def save_metrics_json(model_name: str, score: float):
    ...
    logger.info(f"Validation metrics saved for DVC tracking → {filepath.relative_to(PROJECT_ROOT)}")
...
def save_test_metrics_json(model_name: str, report: dict):
    ...
    logger.info(f"Test metrics saved for DVC tracking → {filepath.relative_to(PROJECT_ROOT)}")
...
def save_baseline_metrics_json(score: float):
    ...
    logger.info(f"Baseline metrics saved for DVC tracking → {filepath.relative_to(PROJECT_ROOT)}")
