...
def load_production_model(alias_name: str = "Production") -> Any:
    ...
    if prefer_local:
        try:
            logger.info("Strategy: PREFER_LOCAL_MODEL=True. Checking local filesystem first...")
...
def preprocess_text_inference(texts: list[str]) -> pd.DataFrame:
    """Preprocesses a list of raw text comments for inference."""
    import nltk
    ...
    cleaned_texts = [DataPreparation.clean_text(t, stop_words) for t in texts]
    df = pd.DataFrame({"clean_comment": cleaned_texts})
    return df


def build_derived_features(df: pd.DataFrame) -> np.ndarray:
    """Alias for FeatureEngineering.build_derived_features to maintain API compatibility."""
    return FeatureEngineering.build_derived_features(df)
